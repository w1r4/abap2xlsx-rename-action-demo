CLASS ycl_ecb_converter_salv_table DEFINITION
  PUBLIC
  INHERITING FROM ycl_ecb_converter_alv
  FINAL
  CREATE PUBLIC .
...
*"* do not include other source files here!!!
  PUBLIC SECTION.
    ...
    METHODS yif_ecb_converter~can_convert_object
        REDEFINITION .
    METHODS yif_ecb_converter~create_fieldcatalog
        REDEFINITION .
    METHODS yif_ecb_converter~get_supported_class
        REDEFINITION .
...
CLASS ycl_ecb_converter_salv_table IMPLEMENTATION.
...
* First update metadata if we can.
    IF io_salv->is_offline( ) = abap_false.
      ...
      ELSE.
        IF ycl_ecb_converter_salv_model=>is_get_metadata_callable( io_salv ) = abap_true.
...
*  ... get the filter information
  ...
  METHOD yif_ecb_converter~get_supported_class.
    rv_supported_class = 'CL_SALV_TABLE'.
  ENDMETHOD.

  METHOD yif_ecb_converter~can_convert_object.
  ...
  METHOD yif_ecb_converter~create_fieldcatalog.
    DATA: lo_salv TYPE REF TO cl_salv_table.

    yif_ecb_converter~can_convert_object( io_object = io_object ).
