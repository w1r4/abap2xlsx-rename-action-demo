CLASS ycl_ecb_legacy_palette DEFINITION
  PUBLIC
  CREATE PUBLIC .
...
*"* do not include other source files here!!!
    ...
    METHODS get_color
      IMPORTING
        !ip_index       TYPE i
      RETURNING
        VALUE(ep_color) TYPE yecb_style_color_argb
      RAISING
        ycx_ecb .
    METHODS get_colors
      RETURNING
        VALUE(ep_colors) TYPE yecb_t_style_color_argb .
    METHODS set_color
      IMPORTING
        !ip_index TYPE i
        !ip_color TYPE yecb_style_color_argb
      RAISING
        ycx_ecb .
  PROTECTED SECTION.
...
*"* do not include other source files here!!!
    ...
    DATA colors TYPE yecb_t_style_color_argb .
ENDCLASS.



CLASS ycl_ecb_legacy_palette IMPLEMENTATION.
  ...
  METHOD get_color.
    ...
    IF sy-subrc <> 0.
      ycx_ecb=>raise_text( 'Invalid color index' ).
    ENDIF.
  ENDMETHOD.
