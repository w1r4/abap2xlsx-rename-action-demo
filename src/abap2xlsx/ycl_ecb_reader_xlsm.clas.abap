CLASS ycl_ecb_reader_xlsm DEFINITION
  PUBLIC
  INHERITING FROM ycl_ecb_reader_2007
  FINAL
  CREATE PUBLIC .
...
CLASS ycl_ecb_reader_xlsm IMPLEMENTATION.
  ...
  METHOD load_vbaproject.
    ...
    ip_excel->yif_ecb_book_vba_project~set_vbaproject( lv_content ).
  ...
  METHOD load_workbook.
    ...
    IF node IS BOUND.
      ...
      io_excel->yif_ecb_book_vba_project~set_codename( fileversion-codename ).
    ENDIF.
    ...
    IF node IS BOUND.
      ...
      io_excel->yif_ecb_book_vba_project~set_codename_pr( workbookpr-codename ).
    ENDIF.
  ...
  METHOD load_worksheet.
    ...
    IF node IS BOUND.
      ...
      IF sheetpr-codename IS NOT INITIAL.
        io_worksheet->yif_ecb_sheet_vba_project~set_codename_pr( sheetpr-codename ).
      ENDIF.
    ENDIF.
  ENDMETHOD.
ENDCLASS.
