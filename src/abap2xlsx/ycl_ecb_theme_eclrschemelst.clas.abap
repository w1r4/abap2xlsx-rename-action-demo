CLASS ycl_ecb_theme_eclrschemelst DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ycl_ecb_theme_eclrschemelst IMPLEMENTATION.
  ...
  METHOD build_xml.
    ...
    IF extracolor IS INITIAL.
      lo_theme_objdef ?= io_document->create_simple_element_ns( prefix = ycl_ecb_theme=>c_theme_prefix
                                                                name   = ycl_ecb_theme=>c_theme_extra_color
                                                                parent = lo_theme ).
  ...
  METHOD load.
    extracolor = ycl_ecb_common=>clone_ixml_with_namespaces( io_extra_color ).
  ENDMETHOD.                    "load
ENDCLASS.
