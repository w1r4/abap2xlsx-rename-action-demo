CLASS ycl_ecb_theme_objectdefaults DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ycl_ecb_theme_objectdefaults IMPLEMENTATION.
  ...
  METHOD build_xml.
    ...
    IF objectdefaults IS INITIAL.
      lo_theme_objdef ?= io_document->create_simple_element_ns( prefix = ycl_ecb_theme=>c_theme_prefix
                                                                name   = ycl_ecb_theme=>c_theme_object_def
                                                                parent = lo_theme ).
    ELSE.
  ...
  METHOD load.
    objectdefaults = ycl_ecb_common=>clone_ixml_with_namespaces( io_object_def ).
  ENDMETHOD.                    "load
ENDCLASS.
